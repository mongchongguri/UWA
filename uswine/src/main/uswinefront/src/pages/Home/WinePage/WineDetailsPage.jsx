...
import { faBox, faDollarSign, faPhone, faHeart as solidHeart } from '@fortawesome/free-solid-svg-icons';
import { faHeart as regularHeart, faThumbsUp } from '@fortawesome/free-regular-svg-icons';
...
function WineDetailComponent({ wineDetail, userinfo }) {
    ...
    const addFavorite = () => {
        AuthApi('/api/mypage/favorite/add', {
            ...
            if (data === 1) {
                setFavorite(true);
                const cart = window.confirm('즐겨찾기 등록 되었습니다. 마이페이지 즐겨찾기로 이동하시겠습니까?');
                if (cart) {
    ...
    const delFavorite = () => {
        AuthApi('/api/mypage/favorite/delete', {
            ...
            if (data === 1) {
                setFavorite(false);
                const cart = window.confirm('즐겨찾기 해제 되었습니다. 마이페이지 즐겨찾기로 이동하시겠습니까?');
                if (cart) {
                    navigate('/mypage/favorite');
                } else {
                    redirect(`/wine/${wineDetail.Id}`);
                            ...
                            <li>
                                {userinfo !== null ? (
                                    !favorite ? (
                                        ...
                                        >
                                            <FontAwesomeIcon icon={regularHeart} size="xl" color="#ff8888" />
                                        </div>
                                    ) : (
                                        <div
                                            onClick={() => {
                                        ...
                                        >
                                            <FontAwesomeIcon icon={solidHeart} size="xl" color="#ff8888" />
                                        </div>
                                    )
                                ) : null}
                            </li>
                                ...
                                return (
                                    <ul className="wine_info_list" key={idx}>
                                        ...
                                        {Array.from({ length: 5 }, (_, i) => {
                                            if (taste[1] > i) {
                                                return <li key={i} className={`wine_detail_taste taste_${i + 1}`}></li>;
                                            } else {
...
function WineSellInfoComponent({ wineDetail, userinfo }) {
    ...
    useEffect(() => {
        ...
        script.onload = () => {
            kakao.maps.load(function () {
                ...
                const imageSrc = 'https://mongchongguriforum.s3.ap-northeast-2.amazonaws.com/maker.png';
    ...
    function cartFunction(store) {
        AuthApi('/api/mypage/cart/buyWine', {
            ...
            if (data === 1) {
                const cart = window.confirm('장바구니에 담았습니다. 장바구니로 이동하시겠습니까?');
                if (cart) {
                    navigate('/mypage/cart');
                }
            } else {
                const cart = window.confirm('이미 장바구니에 있는 항목입니다. 장바구니로 이동하시겠습니까?');
                if (cart) {
                    navigate('/mypage/cart');
                }
            }
    ...
    return (
        <div className="wine_sell_info">
            ...
            <p style={{ color: '#888' }}>현재 상품이 {stores.length}개의 장소에서 판매중입니다.</p>
            <hr />
                            ...
                            ? stores.map(function (store, i) {
                                  ...
                                  return (
                                      ...
                                      >
                                          <li
                                              className={
                                                  clickStates[i] ? 'store_region store_region_click' : 'store_region'
                                              }
                                          >
                                              ...
                                              <ul className="store_wine_info">
                                                  ...
                                                  <li>
                                                      <FontAwesomeIcon className="store_icon" icon={faDollarSign} />
                                                      {store.sellMoney}원
                                                  </li>
                                                  <li>
                                                      <FontAwesomeIcon className="store_icon" icon={faBox} />
                                                      {store.sellStock}
                                                  </li>
                                              </ul>
...
function WineReview({ wineDetail, userinfo }) {
    ...
    useEffect(() => {
        async function fetchData() {
            ...
            Promise.all([...recommendPromises, ...reviewRecommendCount]).then((results) => {
    ...
    const saveReview = () => {
        const userConfirmed = window.confirm('Are you sure you want to save this review?');
        if (userConfirmed) {
    ...
    const updateReview = (props) => {
        const userConfirmed = window.confirm('Are you sure you want to update this review?');
        if (userConfirmed) {
            AuthApi('/api/wine/saveWineReview', {
                id: props,
                mongoId: wineDetail.id,
                                    ...
                                    <li key={i}>
                                        <div className="content_info">
                                            <div className="content_info_title">
                                                <div className="content_info_nickname">{review.nickname}</div>
                                                <div className="content_info_writedate">{review.wineReviewDate}</div>
                                            </div>
                                            <div className="content_info_description">
                                                <div className="content_info_recommend">{reviewRecommendCount[i]}</div>
                                                {reviewRecommend[i] === 1 ? (
                                                    <div
                                        ...
                                        {reviewUpdate && reviewId == review.id ? (
                                            ...
                                            <div className="content_info_content">{review.content}</div>
                                        )}
                                    </li>
                                );
                            })}
