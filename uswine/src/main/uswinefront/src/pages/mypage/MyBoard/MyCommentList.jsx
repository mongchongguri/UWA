...
export default function MyCommentList(){
  ...
  if (token == "") {
    ...
    return <MyCommentListComponent userinfo={userinfo} />;
  }
}
function MyCommentListComponent({userinfo}){
    ...
    useEffect(() => {
        AuthApi("/api/myboard/mycommentList",{
            nickname:userinfo.nickname,
            page : currentPage
        }).then((data)=>{
            console.log(data)
            
            setMyBoardList(data.board)
            if(data.totalPage != 0){
    ...
    function prevPageHandler() {
        if (currentPage > 1) {
          navigate(`/mypage/mycomment/${parseInt(currentPage) - 1}/`);
      ...
      function nextPageHandler() {
        if (currentPage < totalPage) {
          navigate(
            `/mypage/mycomment/${parseInt(currentPage, 10) + 1}/`
          );
