...
function MyInfoComponent({ userinfo }) {
  ...
  useEffect(() => {
    ...
    AuthApi("/api/myinfo/getInfo", {
      ...
      setGetInfo(data);
      let phoneNumber = data.phone.replace(
        /(\d{3})(\d{3,4})(\d{4})/,
        "$1-$2-$3"
      );
  ...
  function nextPageHandler() {
    if (currentPage < totalPage) {
      navigate(`/mypage/myinfo/${parseInt(currentPage, 10) + 1}/`);
            ...
            {openNewNickname ? (
              <>
                ...
                <div className="userinfo_modal_button_box">
                  ...
                  <button className="userinfo_close_modal" onClick={closeModal}>
                    닫기
                  </button>
                </div>
              </>
        ...
        <div className="userinfoBox">
          <div className="userinfo_logo">
            ...
            <div className="userinfo_name">
              {getInfo.nickname} |{" "}
              {getInfo.role == "ROLE_ADMIN"
                ? "관리자"
                : getInfo.role == "ROLE_SELLER"
                ? "판매자"
                : "사용자"}
            </div>
            <div className="userinfo_changeinfo">
              <button className="changeinfo_button" onClick={changeNickname}>
                <FontAwesomeIcon
                  icon={faGear}
                  style={{ color: "#aaa" }}
                  size="l"
              ...
              <button className="changeinfo_button" onClick={changePassword}>
                <FontAwesomeIcon
                  icon={faKey}
                  style={{ color: "#aaa" }}
                  size="l"
          ...
          <div className="userinfo_email">
            ...
            <div className="userinfo_email_value">{getInfo.email}</div>
          </div>
          <div className="userinfo_address">
            ...
            <div className="userinfo_address_value">
              <div className="userinfo_address_basic">{getAddress}</div>
              <div className="userinfo_address_detail">{getDetailAddress}</div>
            </div>
          </div>
        </div>
          ...
          <div className="userinfoMiddleLine">
            <div className="userTotalBoardBox">
              <div className="userTotalBoardBox_label">
                ...
                <div id="userinfo_totalboard_second_label">
                  <Link to={"/mypage/myboard/1"}>전체글보기</Link>
                </div>
              </div>
                ...
                {boardList.length !== 0
                  ? boardList.map(function (board, i) {
                      return (
                        <ul
                          className={
                            i != boardList.length - 1
                              ? "userinfo_board_info_container board_list"
                              : "userinfo_board_info_container board_list userinfo_lastul"
                          }
                          key={i}
                        >
                          <li
                          ...
                          >
                            ...
                            <span className="userinfo_board_comment_count">
                              {boardList_Comment[board.id] +
                                boardList_Recomment[board.id]}
                            </span>
                          </li>
            ...
            <div className="userTotalCommentBox">
              <div className="userTotalBoardBox_label">
                ...
                <div id="userinfo_totalboard_second_label">
                  <Link to={"/mypage/mycomment/1"}>전체글보기</Link>
                </div>
              </div>
              <div className="userTotalBoardBox_value">
                      ...
                      return (
                        ...
                        >
                          ...
                          >
                            {board.title}{" "}
                            <span className="userinfo_board_comment_count">
                              {commentBoardList_Comment[board.id] +
                                commentBoardList_Recomment[board.id]}
                            </span>
                          </li>
                ...
                {buyWineList.length !== 0
                  ? buyWineList.map(function (board, i) {
                      return (
                        <ul
                          className={
                            i != buyWineList.length - 1
                              ? "userinfo_buywinelist_container board_list"
                              : "userinfo_buywinelist_container board_list userinfo_lastul"
                          }
                          key={i}
                          onClick={() => {
